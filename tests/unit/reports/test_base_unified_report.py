...
class TestBaseUnifiedReport:
    def test__normalize_data(self):
        ...
        pd.testing.assert_frame_equal(returned_data['table'], data)
        assert returned_data['table'] is data

    def test__normalize_data_with_dict(self):
        """Test the ``_normalize_data`` method with a data dictionary."""
        # Setup
        ...
        pd.testing.assert_frame_equal(returned_data['table'], data['table'])

    def test__normalize_data_empty_metadata(self):
        """Test the ``_normalize_data`` method with empty metadata."""
        # Setup
        data = pd.DataFrame({'a': [1, 2]})
        metadata = {
            'tables': {},
        ...
        with pytest.raises(ValueError, match=expected_message):
            base_report._normalize_data(data, metadata)

    def test__normalize_data_multi_table_metadata(self):
        """Test the ``_normalize_data`` method with multi-table metadata."""
        # Setup
        data = pd.DataFrame({'a': [1, 2]})
        metadata = {
            'tables': {
        ...
        base_report = BaseUnifiedReport()
        expected_message = (
            'A single pandas.DataFrame can only be used with metadata that contains one table. '
            "Your metadata contains multiple tables: ['table1', 'table2']. "
            'Please either remove the extra tables from the metadata or pass the data as a '
            'dictionary with one pandas.DataFrame per table.'
        )
    ...
    def test__validate_data_format(self):
        ...
        # Setup
        real_dataframe = pd.DataFrame({'column1': [1, 2, 3]})
        synthetic_dataframe = pd.DataFrame({'column1': [1, 2, 3]})

        # Run and Assert
        base_report = BaseUnifiedReport()
        base_report._validate_data_format(real_dataframe, synthetic_dataframe)

    def test__validate_data_format_dict(self):
        """Test the ``_validate_data_format`` method with data dictionaries."""
        # Setup
        real_data = {
    ...
    def test__validate_data_format_error(self):
        """Test the ``_validate_data_format`` method with invalid data."""
        # Setup
        base_report = BaseUnifiedReport()
        real_data = {
            'table1': pd.DataFrame({'column1': [1, 2, 3]}),
        ...
        expected_message = re.escape(
            ...
            'pandas.DataFrame. Received real_data=dict and synthetic_data=DataFrame.'
        )
        with pytest.raises(ValueError, match=expected_message):
            base_report._validate_data_format(real_data, synthetic_data)
    ...
    def test__validate_metadata_error(self, mock__validate_metadata):
        ...
        base_report._normalize_data = Mock()
        ...
        base_report._normalize_data.assert_not_called()
