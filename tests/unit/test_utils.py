...
@pytest.mark.parametrize(
    ('parent', 'child'),
    [
        [pd.Series([1, 2, 3, 4, 5]), pd.Series([3, 4, 1, 4, 4, 5, 1])],
        [
            pd.DataFrame({'id0': ['id0', 'id0', 'id1', 'id1', None], 'id1': [0, None, None, 1, 0]}),
            pd.DataFrame({
                'fk0': ['id0', 'id0', 'id1', 'id1', 'id1', 'id1', None],
                'fk1': [0, 0, None, 1, 1, 1, 0],
            }),
        ],
    ],
)
def test_get_cardinality_distribution(parent, child):
    """Test the ``get_cardinality_distribution`` utility function."""
    # Run
    cardinality_distribution = get_cardinality_distribution(parent, child)
